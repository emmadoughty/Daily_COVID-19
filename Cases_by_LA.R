...
#### Awaiting_Clarification plots ####
#Read in new cases per day from summarised data, format date correctly
Awaiting_Clarification_data <- data.frame(Date=c(COVID19_by_LA$Date), ntimes=c(COVID19_by_LA$Awaiting_Clarification))
Awaiting_Clarification_data <- as.data.frame(lapply(Awaiting_Clarification_data, rep, Awaiting_Clarification_data$ntimes))


# Plot number of NEW CASES per day
Awaiting_ClarificationNew_Sigfig <- signif(max(COVID19_by_LA$Awaiting_Clarification))
Awaiting_ClarificationNew_Factor <- 10^(floor(log10(Awaiting_ClarificationNew_Sigfig)))
Awaiting_ClarificationNew_Max <- round_any(max(COVID19_by_LA$Awaiting_Clarification), Awaiting_ClarificationNew_Factor, f=ceiling)
Awaiting_ClarificationNew_Breaks <- ceiling(Awaiting_ClarificationNew_Factor/2)

plot_Awaiting_ClarificationNew <- ggplot(data=Awaiting_Clarification_data, aes(x=Awaiting_Clarification_data$Date)) +
  geom_histogram(binwidth=1, boundary = 1, colour="black", fill= "#E5E1EE") +
  scale_x_date(labels = date_format("%d/%m/%y"), expand = c(0, 0), 
               limits = c(min(COVID19_by_LA$Date)-1, max(COVID19_by_LA$Date)),
               breaks = seq(min(COVID19_by_LA$Date)-1, max(COVID19_by_LA$Date), 1)) +
  scale_y_continuous(limits = c(0, Awaiting_ClarificationNew_Max), 
                     breaks = seq(0, Awaiting_ClarificationNew_Max, Awaiting_ClarificationNew_Breaks),
                     expand = c(0, 0)) +
  ylab("Frequency") + xlab("Date") +
  ggtitle("New cases per day: Awaiting Clarification") +
  theme_minimal() +
  theme(plot.title = element_text(size=13, face="bold"),
        axis.line = element_line(colour = "black"),
        axis.text.x = element_text(angle = 70, hjust = 1, vjust = 1), 
...
# Plot CUMULATIVE CASES per day
Awaiting_ClarificationCum_Sigfig <- signif(sum(COVID19_by_LA$Awaiting_Clarification))
Awaiting_ClarificationCum_Factor <- 10^(floor(log10(Awaiting_ClarificationCum_Sigfig)))
Awaiting_ClarificationCum_Max <- round_any(sum(COVID19_by_LA$Awaiting_Clarification), Awaiting_ClarificationCum_Factor, f=ceiling)
Awaiting_ClarificationCum_Breaks <- ceiling(Awaiting_ClarificationCum_Factor/2)

plot_Awaiting_ClarificationCum <- ggplot(data=Awaiting_Clarification_data, aes(x=Awaiting_Clarification_data$Date)) + 
  geom_histogram(aes(y = cumsum(..count..)), binwidth=1, 
                 boundary = 1, colour="black", fill= "#DFFDFF") +
  scale_x_date(labels = date_format("%d/%m/%y"), expand = c(0, 0), 
               limits = c(min(COVID19_by_LA$Date)-1, max(COVID19_by_LA$Date)),
               breaks = seq(min(COVID19_by_LA$Date)-1, max(COVID19_by_LA$Date), 1)) +
  scale_y_continuous(limits = c(0, Awaiting_ClarificationCum_Max), 
                     breaks = seq(0, Awaiting_ClarificationCum_Max, Awaiting_ClarificationCum_Breaks), expand = c(0, 0)) +
  ylab("Cumulative frequency") + xlab("Date") +
  ggtitle("Cumulative cases: Awaiting Clarification") +
  theme_minimal() +
  theme(plot.title = element_text(size=13, face="bold"),
        axis.line = element_line(colour = "black"),
        axis.text.x = element_text(angle = 70, hjust = 1, vjust = 1), 
...
# Generate figures
Awaiting_Clarification_figure <- ggarrange(plot_Awaiting_ClarificationNew + font("x.text", size = 8), 
                                           plot_Awaiting_ClarificationCum + font("x.text", size = 8), 
                                           ncol = 1, nrow = 2, align = "hv")

pdf("Awaiting_Clarification_cases_plot.pdf", height = 8.27, width = 11.69)
Awaiting_Clarification_figure
dev.off()
